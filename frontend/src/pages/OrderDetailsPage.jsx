  ...
  return (
    <>
      {isLoading ? (
        ...
        <Message variant='danger' style={fadeInStyle}>
          {error?.data?.message || error.error}
        </Message>
      ) : (
        <>
          <Meta title={'Order Details'} />
          <h1 style={fadeInStyle}>Order ID: {orderId}</h1>
          <Row>
            <Col md={8}>
              <ListGroup variant='flush'>
                <ListGroup.Item style={transitionStyle}>
                  <h2>Shipping </h2>
                  <div className='mb-3'>
                    <strong>Name:</strong> {order?.user?.name}
                  </div>
                ...
                <ListGroup.Item style={transitionStyle}>
                  ...
                  <ListGroup variant='flush'>
                    {order?.orderItems?.map(item => (
                      <ListGroup.Item key={item._id} style={hoverStyle}>
                        <Row>
                          <Col md={2}>
                            <Image
                              src={item.image}
